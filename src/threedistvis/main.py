...
instructions = {
    'Power BI': (
        "Import distributions_data.csv. For Graph 1: Use Clustered Column Chart, bin 'value' (50 bins), count 'value', legend 'color'. "
        "Overlay PDFs with Line Chart using DAX for norm.pdf. For Graph 2: Histogram of 'value' (grey), overlay PDFs with 'predicted_color'."
    ),
    'Streamlit': (
        "Save script as app.py, install streamlit, run 'streamlit run app.py'. Displays both graphs. "
        "Use st.pyplot for graphs or st.image for PNG. Data in distributions_data.csv."
    ),
    'Tableau': (
        "Import distributions_data.csv. Graph 1: Histogram of 'value' (50 bins), color by 'color', dual-axis for PDFs (calculated fields). "
        "Graph 2: Histogram of 'value' (grey), overlay PDFs by 'predicted_color'."
    )
}

# Generate one PNG per tool
for tool, instruction in instructions.items():
    fig = plt.figure(figsize=(8, 12))  # Portrait aspect ratio

    # First subplot: True distributions
    ...
    for i, color in enumerate(colors):
        ...
        ax1.plot(x, pdf, color=color, linestyle='--', label=f'{color.capitalize()} PDF')
    ...
    # Second subplot: All samples with estimated PDFs
    ...
    for i, color in enumerate(colors):
        ...
        if len(subset) > 0:
            ...
            ax2.plot(x, pdf, color=color, linestyle='--', label=f'{color.capitalize()} Estimated PDF')
    ...
    # Add tool name and instructions
    ...
    plt.savefig(f'{tool.lower()}.png', dpi=300, bbox_inches='tight')
    plt.close()

print("Generated powerbi.png, streamlit.png, tableau.png")
